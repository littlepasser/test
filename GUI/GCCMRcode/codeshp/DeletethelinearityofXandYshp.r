...
# ({min1},{max1},{intreval1},{E1},\"{xName}\",\"{yName}\",\"{coordsX_Name}\",\"{coordsy_Name}\",{cores1},{formattedselectshppath},{formattedSaveshpPath})
###########################run GCCMshp#########################################################
library(parallel)
library(foreach)
library(doParallel)
...
GCCMshpmainfunction<-function(minlib,maxlib,intervalib,E,xName,yName,coordsX_Name,coordsY_Name,cores,formattedselectshppath,formattedSaveshpPath)
{
 ...
 x_xmap_y <- GCCMLattice(embedings, y, lib_sizes, lib, pred, E,tau,b,cores) #predict y based on x  
  ...
  # save(minlib,maxlib,intervalib,E,xName,yName,coordsX_Name,coordsY_Name,cores,lib_sizes,
  #      x_xmap_y_means,y_xmap_x_means,x_xmap_y_Sig,y_xmap_x_Sig,x_xmap_y_interval,y_xmap_x_interval,file = formattedSaveshpPath)
